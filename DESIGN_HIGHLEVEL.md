...
subgraph Cloud["Cloud Services"]
    ...
    DATA["Telemetry & Event Layer"]
    
end

subgraph Field["DER Sites"]
    DER["DER Assets<br/>(Inverters, BESS, PV)"]
    EDGE["Edge Control Agents<br/>(Local Safety & Control)"]
end

subgraph Legacy["OT Environment"]
    SCADA["Legacy SCADA"]
    UI["Market UI"]
    ICCP["ICCP / TASE.2"]
end

DER <-- Local Control --> EDGE
EDGE -- initiate --> CP
CP -- response --> EDGE
EDGE --> DATA
CP <--> DATA
...
SCADA -->|Outbound Session Initiation| FW
FW -->|Permitted, Stateful| CP
CP --> DATA
